...
sv <- c(rep(1,4), rep(2,16), rep(3,90), rep(4,70), rep(5,20), rep(6, 100))
plot.ecdf(sv)
sv

# EDCF for continuous variables
plot.ecdf(time)
plot.ecdf(temperature)
plot.ecdf(NewTemperature)

pizza$DeliveryTime <- ifelse(pizza$time <= 10, '[0; 10]', 
                             ifelse(pizza$time <= 15, '(10; 15]',
                             ifelse(pizza$time <= 20, '(15; 20]',
                             ifelse(pizza$time <= 25, '(20; 25]',
                             ifelse(pizza$time <= 30, '(25; 30]',
                             ifelse(pizza$time <= 35, '(30; 35]',
                             ifelse(pizza$time <= 40, '(35; 40]',
                             ifelse(pizza$time <= 45, '(40; 45]',
                             ifelse(pizza$time <= 50, '(45; 50]',
                             ifelse(pizza$time <= 55, '(50; 55]',
                                    '.'))))))))))
...
library(ggplot2)
ggplot(pizza, aes(time)) + stat_ecdf(geom = "point")
ggplot(pizza, aes(time)) + stat_ecdf(geom = "step")

# Bar Chart
barplot(table(branch))
barplot(table(branch)/length(branch))
barplot(table(DeliveryTime), horiz=TRUE)
...
plot(density(example, kernel = 'rectangular'))
plot(density(example, kernel = 'epanechnikov'))

plot(density(time, kernel = 'gaussian'))
plot(density(time, kernel = 'rectangular'))
plot(density(time, kernel = 'epanechnikov'))

hist(time, # histogram
     col="peachpuff", # column color
     border="black",
     freq = F, # show densities instead of frequencies
     xlab = "Delivery time")
lines(density(time, kernel = 'gaussian'), # density plot
...
barplot(results2014,
        names.arg=c('ANC',"DA","EFF","IFP","COPE","Others"),
        col=gray.colors(6),
        ylim=c(0,0.7),
        xlab="Parties",
        ylab="Votes (%)",
...
par(mar= c(5, 5, 2, 2))
hist(goals, breaks=c(0,15,30,45,60,75,90,96),col="lightgrey", 
     cex.axis=1.75,lwd=3,cex.lab=1.75,cex.main=1.75,ylab="Density",xlab="Time (in min)",
     xlim=c(0,100),ylim=c(0,0.025),main="")
dev.off()
...
par(mar= c(5, 5, 2, 2))
plot.ecdf(as.numeric(as.character(goals_cat)), cex.axis=1.75,lwd=3,
          cex.lab=1.75,cex.main=1.75,xlab="Time (in min)",ylab="F(x)",main="")
dev.off()
...
par(mar= c(5, 5, 2, 2))
plot.ecdf(score, cex.axis=1.75,lwd=3,cex.lab=1.75,cex.main=1.75,
          xlab="Score",ylab="F(x)",main="",yaxp=c(0,1,10),xaxp=c(0,10,10))
...
par(mar= c(5, 5, 2, 2))
hist(temperature, cex.axis=1.75,lwd=3,cex.lab=1.75,cex.main=1.75,
     xlab="Temperature",main="",xlim=c(40,90),ylim=c(0,400),col="lightgrey",ylab="Number of deliveries")
...
p2 <- p1 + geom_histogram(fill="darkgrey",alpha=0.5,binwidth=2.5,) + 
  scale_y_continuous("Number of deliveries") + scale_x_continuous("Temperature")

plot(p2 + theme(axis.title.x = element_text(size=20),
...
par(mar= c(5, 5, 2, 2))
barplot(table(driver),ylim=c(0,400),
        ylab="Number of deliveries", 
        xlab="Driver",
        col=gray.colors(7),
        main="Title", cex.axis=1.75,lwd=3,cex.lab=1.75,cex.main=1.75,cex.names=1.5)
dev.off()


# d)  [the "theme" option is to improve labelling and font size, not necessarily needed]
qplot(driver,data=pizza)

pdf(file="exercise_2.6_d.pdf",width=11)
p3 <- qplot(driver,data=pizza,fill=day)
p4 <- p3 + scale_fill_grey() +theme_bw() + 
  scale_y_continuous("Number of deliveries") + 
  scale_x_discrete("Driver") 
plot(p4 + theme(axis.title.x = element_text(size=20),
               axis.title.y = element_text(size=20), 
